"""
voxd.paths - all filesystem look-ups live here.
"""
...
from voxd.utils.libw import diagn
...
def _locate_whisper_cli() -> Path:
    ...
    Order (first hit wins):
      1. ``$VOXD_WC_BIN`` – explicit override.
    ...
    raise FileNotFoundError(
        "Could not locate *whisper-cli*. Checked $VOXD_WC_BIN, repo-local build and $PATH. "
...
def _locate_llama_server() -> Path:
    ...
    Order (first hit wins):
      1. $VOXD_LLAMA_SERVER_BIN – explicit override
    ...
    # 1. Environment override
    env = os.getenv("VOXD_LLAMA_SERVER_BIN")
    if env:
        p = Path(env).expanduser().resolve()
        if p.is_file():
            return p
    ...
    raise FileNotFoundError(
        "Could not locate llama-server. Checked $VOXD_LLAMA_SERVER_BIN, repo-local build and $PATH. "
...
def _locate_llama_cli() -> Path:
    ...
    # 1. Environment override
    env = os.getenv("VOXD_LLAMA_CLI_BIN")
    if env:
        p = Path(env).expanduser().resolve()
        if p.is_file():
            return p
    ...
    raise FileNotFoundError(
        "Could not locate llama-cli. Checked $VOXD_LLAMA_CLI_BIN, repo-local build and $PATH."
    )
...
def _locate_default_llamacpp_model() -> Path:
    ...
    # 1. Environment override
    env = os.getenv("VOXD_LLAMACPP_MODEL_PATH")
    if env:
