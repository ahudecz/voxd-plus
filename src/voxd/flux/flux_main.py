...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="voxd --flux", description="VOXD Flux: VAD-triggered continuous dictation")
    # Flux VAD options
...
class FluxRunner:
    def __init__(self, cfg: AppConfig, *, min_silence_ms: int | None, min_speech_ms: int | None, pre_roll_ms: int | None, save_audio: bool, debug_vad: bool = False, no_resample: bool = False, monitor: bool | None = None, calib_sec: float | None = None, noise_suppress: bool | None = None):
        ...
        self.transcriber = WhisperTranscriber(
            ...
            cfg.whisper_binary,
            delete_input=not self.save_audio,
    ...
    def run(self):
        ...
        try:
            with sd.InputStream(samplerate=self.fs, channels=1, dtype="float32", blocksize=self.N, callback=self._callback, device=dev_pref if dev_pref else None):
                ...
                if self.monitor_enabled:
                    try:
                        from voxd.flux.flux_gui import show_gui
